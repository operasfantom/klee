...
#define addDNR(name, handler) { name, \
  ...
  add("__ubsan_handle_type_mismatch_v1_abort", handleTypeMismatch, false),
  ...
  add("__ubsan_handle_nonnull_return", handleNonnullReturn, false),
  add("__ubsan_handle_nonnull_return_abort", handleNonnullReturn, false),
  add("__ubsan_handle_nonnull_return_v1", handleNonnullReturn, false),
  add("__ubsan_handle_nonnull_return_v1_abort", handleNonnullReturn, false),
...
void SpecialFunctionHandler::handleTypeMismatch(
    ExecutionState &state, KInstruction *target,
    std::vector<ref<Expr>> &arguments) {
  ...
  if (pointer->isZero()) {
    ...
    executor.terminateStateOnError(
        state,
        "either misaligned address for 0x" + Twine::utohexstr(pointerAddress) +
            " or invalid usage of address 0x" +
            Twine::utohexstr(pointerAddress) + " with insufficient space",
        Executor::UndefinedBehavior);
  }
}
...
void SpecialFunctionHandler::handleAlignmentAssumption(
  ...
  if (offset->isZero()) {
    executor.terminateStateOnError(state,
                                   "assumption of " + Twine(alignmentValue) +
                                       " byte alignment failed",
                                   Executor::UndefinedBehavior);
  } else {
    executor.terminateStateOnError(state,
                                   "assumption of " + Twine(alignmentValue) +
                                       "  byte alignment (with offset of " +
                                       Twine(offsetValue) +
                                       " byte) failed",
                                   Executor::UndefinedBehavior);
  }
...
void SpecialFunctionHandler::handleNonnullReturn(
    ExecutionState &state, KInstruction *target,
    std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(
      state,
...
void SpecialFunctionHandler::handlePointerOverflow(
  ...
  uint64_t resultValue = result->getZExtValue();
  if (base->isZero() && result->isZero()) {
    // Check is available since LLVM 10.0
    executor.terminateStateOnError(state,
    ...
    executor.terminateStateOnError(state,
                                   "applying non-zero offset " +
                                       Twine(resultValue) + " to null pointer",
                                   Executor::UndefinedBehavior);
  } else if (!base->isZero() && result->isZero()) {
    executor.terminateStateOnError(
        state,
        "applying non-zero offset to non-null pointer 0x" +
            Twine::utohexstr(baseValue) + " produced null pointer",
        Executor::UndefinedBehavior);
  } else {
    executor.terminateStateOnError(
        state,
        "pointer arithmetic with base 0x" + Twine::utohexstr(baseValue) +
            " overflowed to 0x" + Twine::utohexstr(resultValue),
        Executor::UndefinedBehavior);
  }
}
